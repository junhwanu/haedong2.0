...
class ChartManger(__manager.ManagerClass):
        ...
        try:
            ...
            if self.sbv.info[subject_code]['전략'] == 파라:
                for chart_config in self.stv.info[subject_code][파라][차트]:
                    ...
                    if chart_type == 틱차트:
                        if 틱차트 not in self.data[subject_code]:
                        ...
                        if MODE == REAL:
                            ...
                            self.data[subject_code][chart_type][time_unit][시가] = []
                            self.data[subject_code][chart_type][time_unit][고가] = []
                            self.data[subject_code][chart_type][time_unit][저가] = []
                        ...
                        self.data[subject_code][chart_type][time_unit][이동평균선]['일수'] = self.stv.info[subject_code][self.sbv.info[subject_code]['전략']][차트변수][chart_type][time_unit][이동평균선]
    ...
    def init_current_candle(self, subject_code, chart_type, time_unit):
        s_time = time.time()
        self.data[subject_code][chart_type][time_unit]['현재캔들'][고가] = 0
        self.data[subject_code][chart_type][time_unit]['현재캔들'][저가] = INFINITY

        if chart_type is 틱차트:
            self.data[subject_code][chart_type][time_unit]['현재가변동횟수'] = 0
        elif chart_type is 분차트:
            self.data[subject_code][chart_type][time_unit]['현재가변동시간'] = '?????'
    ...
    def push(self, subject_code, chart_type, time_unit, candle):
        try:
            ...
            self.data[subject_code][chart_type][time_unit][현재가].append(candle[현재가])
            self.data[subject_code][chart_type][time_unit][시가].append(candle[시가])
            self.data[subject_code][chart_type][time_unit][고가].append(candle[고가])
            self.data[subject_code][chart_type][time_unit][저가].append(candle[저가])
            self.data[subject_code][chart_type][time_unit][체결시간].append(candle[체결시간])
        ...
        for days in self.data[subject_code][chart_type][time_unit][이동평균선]['일수']:
            if self.data[subject_code][chart_type][time_unit]['인덱스'] >= days - 1:
                if self.data[subject_code][chart_type][time_unit]['인덱스'] == days - 1:
                    ...
                    alpha = 2 / (days + 1)
                    ema = alpha * self.data[subject_code][chart_type][time_unit][현재가][-1] + (1.0 - alpha) * \
                                                                                              self.data[subject_code][chart_type][
    ...
    def init_sar(self, subject_code, chart_type, time_unit):
        ...
        af = self.stv.info[subject_code][파라][차트변수][chart_type][time_unit][INIT_AF]
        ...
        for i in range(idx):
            temp_high_price_list.append(self.data[subject_code][chart_type][time_unit][고가][i])
            temp_low_price_list.append(self.data[subject_code][chart_type][time_unit][저가][i])
    ...
    def calc_sar(self, subject_code, chart_type, time_unit):
        ...
        init_af = self.stv.info[subject_code][파라][차트변수][chart_type][time_unit][INIT_AF]
        max_af = self.stv.info[subject_code][파라][차트변수][chart_type][time_unit][MAX_AF]
        ...
        if temp_flow == "상향":
            if self.data[subject_code][chart_type][time_unit][저가][index] >= next_sar:  # 상승추세에서 저가가 내일의 SAR보다 높으면 하락이 유효
                today_sar = next_sar
                temp_flow = "상향"
                the_lowest_price = 0
                if self.data[subject_code][chart_type][time_unit][고가][index] > ep:  # 신고가 발생
                    the_highest_price = self.data[subject_code][chart_type][time_unit][고가][index]
                    ep = self.data[subject_code][chart_type][time_unit][고가][index]
                    af = af + init_af
                    if af > max_af:
                        af = max_af

            elif self.data[subject_code][chart_type][time_unit][저가][index] < next_sar:  # 상승추세에서 저가가 내일의 SAR보다 낮으면 하향 반전
                temp_flow = "하향"
                af = init_af
                today_sar = ep
                the_highest_price = 0
                the_lowest_price = self.data[subject_code][chart_type][time_unit][저가][index]

                ep = the_lowest_price
                self.log.info("%s, %s, %s : 하향 반전, 시간 : %s" % (
                    subject_code, chart_type, time_unit, self.data[subject_code][chart_type][time_unit][체결시간][-1]))
            ...
            if self.data[subject_code][chart_type][time_unit][고가][index] <= next_sar:  # 하락추세에서 고가가 내일의 SAR보다 낮으면 하락이 유효
                today_sar = next_sar
                temp_flow = "하향"
                the_highest_price = 0
                if self.data[subject_code][chart_type][time_unit][저가][index] < ep:  # 신저가 발생
                    the_lowest_price = self.data[subject_code][chart_type][time_unit][저가][index]
                    ep = self.data[subject_code][chart_type][time_unit][저가][index]
                    af = af + init_af
                    if af > max_af:
                        af = max_af

            elif self.data[subject_code][chart_type][time_unit][고가][index] > next_sar:  # 하락추세에서 고가가 내일의 SAR보다 높으면 상향 반전
                temp_flow = "상향"
                af = init_af
                today_sar = ep
                the_lowest_price = 0
